...
> - **点歌次数可见 + 抽屉开时重拉曲库(2026-08-06)**:此前曲库只在**连接时**拉一次,本场点过的歌
>   服务端次数涨了、手机列表却按旧数据排,看着像"没按次数排序";且次数不可见,大量同次数歌按歌名排
>   也显得乱。现改:①打开点歌抽屉即 `refreshLibrary()` 重拉(次数/排序保持最新);②每行右侧显示
>   "N次"(0 次不显示,免噪音);③点歌成功本地 `plays` 即时 +1(**不重排**,避免正在浏览的列表行
>   跳动,新顺序下次开抽屉生效)。
