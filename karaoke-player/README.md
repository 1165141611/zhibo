...
| 礼物菜单 | 绿幕**左侧竖排"礼物→权益"引导条**(抖音礼物图标 + 自定义文字,如 🎈点歌 / 🍰插队);**不透明深色圆角底板 + 黑 keyline**(彩色半透明礼物 PNG 必须坐底板上,裸贴绿会留绿边);`G` 显隐、**鼠标可单独拖动摆放**(命中检测:按在礼物条上拖它、否则拖整窗);内容由 pc-service「礼物菜单配置」窗选,显隐/位置缓存 |
...
- **礼物菜单必须坐在不透明底板上**(2026-08-05):抖音礼物图是**彩色半透明 PNG**,直接贴纯绿
  背景时,软边(半透明像素)会与绿混合 → 直播伴侣抠像后留一圈**绿边**;不少礼物图本身还带绿/青
  色块会被抠穿。修法与歌词/音准线同源:每个礼物预合成一张**不透明深色圆角底板 + 最外一圈黑 keyline**
  的卡片(图标+自定义文字都画在底板上,抗锯齿边缘落黑上、绿幕干净抠;文字坐不透明底板故白字无需描边)。
  卡片**预合成成 QPixmap 缓存一次**(内容/字体/DPR 变才重建,同 setlist 的 GIL 守卫),`paintEvent` 只
  blit——礼物图缩放 + `drawText`(emoji 走它才出彩色)都是 GUI 线程重活,绝不能每帧做。见 `_build_gift_pix`。
...
- `STATE {json}` —— 每 500ms:`pos/dur`(ms)、`playing`、`key`、`vocal`、`vol`(0-100)、`pitch`(音准线显隐)、`font`(字体索引)、`setlist_show`(歌单显隐)、`setlist_y`(歌单位置)、`gifts_show`(礼物菜单显隐)、`gift_x`/`gift_y`(礼物条位置)、`win_x`/`win_y`(窗口桌面位置)、`mid`、`title`、`artist`
