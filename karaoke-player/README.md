...
- **音准线压扁**:高度 = `2×行高`(`≤两行歌词`),放窗口上部一条扁平窄带,只表示旋律趋势。
  **粗细/描边与歌词看齐(2026-07-15)**:`bar_h=PITCH_BAR_H=6` + **黑描边 `PITCH_OW=2`**(不再是旧的淡化
  ...
  只要求改粗细)。
- **音高游标(2026-07-15,替代起唱竖线)**:`_draw_pitch_cursor` 在播放头 x 处画**白色实心亮点 + 径向渐变
  柔和光晕**(白亮心→外圈透明);y 高度=**当前时刻所在音符的音高**,每帧朝目标插值(像弹跳小球平滑上下),
  **无音符(间奏/尾奏)→ 平滑落到底部等待**。直径≈`1.3×音准线总粗`(稍大于音准线粗细)。实心白核绿幕干净抠,
  光晕靠直播伴侣抠像羽化合成。原"起唱白竖线"已移除。
