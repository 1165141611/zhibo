...
- **礼物菜单靠"剪影描边"上绿幕,不用底板**(2026-08-05 加,08-06 由底板改描边):抖音礼物图是
  ...
  **贴纸式黑描边**:把图标+白字画到透明"内容层"→ 取其 alpha 填黑得"黑剪影"(`CompositionMode_SourceIn`)→
  ...
  描边**宽/颜色/间距均可调**(08-06 三次演进:底板→黑描边→细淡→**做成配置窗可调项**):
  `gift_outline`(0~3px)、`gift_color`(不透明色)、`gift_gap`(0~24px)、`gift_scale`(0.4~2.0)都是实例属性,
  pc-service「礼物菜单配置」窗滑块/取色器控、经 IPC 下发 + STATE 回读缓存。**描边色用不透明**(剪影填该色、
  8 向偏移画一圈):opaque 才绿幕干净(不留暗绿边),**"淡化"靠选浅灰而非半透明**(默认深灰 `#333`)。见 `_build_gift_pix`。
- **升降调必须是实时时域 WSOLA,不能用相位声码器,不能分块**:
  ...
  - 正解 = **单个持续状态的 WSOLA 贯穿整流**。用 `audiotsm.wsola` 的 `read_from/write_to`(维护
    ...
    重采样已**向量化**(2026-08-07:原逐样本 Python 循环是变调时生产者最大 CPU 头,numpy
    线性插值提速 ~50×,更抗别的进程挤压;配套 pc-service 侧优先级修正,见 live-remote
    DEV_LOG 二十四)。
...
| `gift_outline <f>` | 描边宽(0~3px,0=不描边;配置窗滑块,缓存) |
| `gift_gap <n>` | 卡片竖直间距(0~24px,绝对;配置窗滑块,缓存) |
| `gift_color <#rgb>` | 描边颜色(`#rrggbb`,不透明;配置窗取色器,缓存) |
