...
| 礼物菜单 | 绿幕**左侧竖排"礼物→权益"引导条**(抖音礼物图标 + 自定义文字,如 🎈点歌 / 🍰插队);**无底板,图标/文字各自描一圈黑边**(贴纸式轮廓——取内容剪影填黑、8 向偏移画一圈;彩色半透明礼物 PNG/文字裸贴绿会留绿边,描边把抗锯齿边缘落黑上、绿幕干净抠;emoji 也一体描边);`G` 显隐、**鼠标可单独拖动摆放**(命中检测)、**尺寸可调**(配置窗滑块 40~200%);描边粗细看齐歌单(细);内容由 pc-service「礼物菜单配置」窗选,显隐/位置/尺寸缓存 |
...
- **礼物菜单靠"剪影描边"上绿幕,不用底板**(2026-08-05 加,08-06 由底板改描边):抖音礼物图是
  **彩色半透明 PNG**,直接贴纯绿时软边(半透明像素)与绿混合 → 抠像留一圈**绿边**,图内绿/青块还会被抠穿;
  白字裸贴绿同理。**初版**给每个礼物垫**不透明深色圆角底板**隔绝(有效但视觉重);**现版**去底板,改
  **贴纸式黑描边**:把图标+白字画到透明"内容层"→ 取其 alpha 填黑得"黑剪影"(`CompositionMode_SourceIn`)→
  在 8 个方向各偏移 `r` 画一遍 = 一圈黑轮廓,再盖回内容层。等价歌词/音准线的黑 keyline(抗锯齿边缘落黑上、
  绿幕干净抠),只是描的是**任意位图剪影**而非字形路径——故 emoji(彩色,drawText 渲染)也能统一描边。
  卡片**预合成 QPixmap 缓存一次**(内容/字体/DPR/尺寸变才重建,同 setlist 的 GIL 守卫),`paintEvent` 只 blit
  ——图标缩放 + drawText + 剪影合成都是 GUI 线程重活,绝不每帧做。尺寸 `gift_scale`(0.4~2.0)缩放全部量;
  描边宽 `GIFT_OUTLINE=2`(剪影外扩 r,全部外露)**看齐歌单**——歌单是居中笔画 `OW_BLACK(6)×font_small/font_big=4`
  外露约 2px,故 r=2 与之相当,别更粗。见 `_build_gift_pix`。
...
| `gift_scale <f>` | 礼物菜单整体缩放(0.4~2.0,越界夹取;配置窗滑块调,pc-service 缓存) |
...
- `STATE {json}` —— 每 500ms:`pos/dur`(ms)、`playing`、`key`、`vocal`、`vol`(0-100)、`pitch`(音准线显隐)、`font`(字体索引)、`setlist_show`(歌单显隐)、`setlist_y`(歌单位置)、`gifts_show`(礼物菜单显隐)、`gift_x`/`gift_y`(礼物条位置)、`gift_scale`(礼物菜单缩放)、`win_x`/`win_y`(窗口桌面位置)、`mid`、`title`、`artist`
