...
| `segment_pitch.py` | **分段变调工具**(离线 CLI,2026-08-14):把曲库某歌**指定时间段**升/降调,生成新曲目入库、原曲不动(典型:副歌太高单独降全音、主歌保持原调)。`analyze` 打印逐句时间轴+音高辅助挑边界;`build` 用 stftpitchshift 相位声码器(输出**严格等长**,歌词/音准时间轴零漂移)移调、段边界放唱句间隙、1.2s 等功率交叉淡化衔接、RMS 响度补偿(限峰按 99.99 分位,防声码器个别相位尖峰把整段增益卡低 1~2dB);伴奏/原唱两轨同处理,`.note` 段内音高同步移调。入库带 `named:true` 防启动迁移改标题;**生成后须重启 pc-service**(清单 library.json 启动载入,服务运行中外部改动会被内存覆盖)。**标记约定**:歌名尾缀方括号(如「再也不会去丽江[副歌-2]」)——托盘/手机列表可见标记,播放器绿幕(开头标题卡+顶端滚动歌单)经 `player._viewer_title` **剥掉结尾方括号**不给观众展示(只剥结尾防误伤真带括号歌名);**不禁整首升降调**(实时 WSOLA 叠加在烘焙音频上,整体升降不破坏衔接)。清单/meta 记 `source_mid`/`pitch_segments`/`pitch_fade` 留档,微调过渡点时删目录按参数重跑即可复现。首例:「再也不会去丽江[副歌-2]」三段副歌各降全音。 |
