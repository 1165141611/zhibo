...
## 二十三、"少年锦时 315 次"点歌计次污染破案 + 防抖守卫(2026-08-06)
> 注:本节原误编号"二十一"(与样式窗那节撞号),2026-08-07 更正为二十三。
**症状**:手机点歌列表里"少年锦时"显示点了 315 次,第二名"家乡"才 15 次,断崖式异常。
**排查**:`bump_play`(计次唯一入口)全仓只有 `k_enqueue`(WS `kqueue_add`)一条触发路径——
手机 App 无重发/循环、托盘"播放"走 `k_play_mid` 不计次、队列不持久化不回放、网页测试端不发点歌。
**根因(历史污染,非现行 bug)**:第十五节修"点歌卡顿"(2026-07-23)等调试用过走真实
`k_enqueue→bump_play` 全路径的压测/冒烟脚本(scratchpad 临时件,已删)。脚本反复拿"列表第一首"
入队,而列表恰按 plays 倒序——第一次 +1 后该曲钉死第一位,后续每轮压测全灌给它(**正反馈**)。
- **数据修复**:真实次数无日志可还原,按作者要求把该曲 plays 改成 5(停服务后改
  `library.json` 再重启,防内存态整表回写冲掉)。
- **守卫**:`library.bump_play` 加防抖——同一 mid 在 `_BUMP_DEBOUNCE_S=10` 秒内重复入队
  **不重复计次**(入队本身照常),防手滑连点、防将来脚本循环污染。沙盒验证:立即重复不计、
  超窗口正常 +1、落盘正确。
- **教训**:压测/冒烟脚本走真实业务路径时,**凡有持久化副作用(计次/缓存/清单)必须指向沙盒
  副本**,或给副作用加可辨识的旁路;否则测试流量混进真实统计,事后无法区分。

## 二十四、开托盘窗瞬间正在播的伴奏"咯噔"根治 —— 进程/线程优先级 + 重采样向量化(2026-08-07)
**症状**:播放器唱着歌时,一开托盘子窗口(曲库管理/扫描导入等)伴奏就短暂卡一下;窗口越重越明显。
**根因(跨进程 CPU 调度挤压,三连击)**:
1. 开窗瞬间 pc-service 有整核级 CPU 尖峰:曲库窗要在 Tk UI 线程同步建首批 60 行控件(每行勾选框+
   输入框+3 按钮,400+ 个 Tcl 控件,200~500ms);扫描窗还跑 `adb devices`。
2. 新窗口置顶+抢焦点 → pc-service 瞬间变**前台进程**,吃 Windows 前台调度加成(更长时间片)——
   正在烧 CPU 的进程恰好还优先调度。
3. 播放器的声卡回调是 Python 函数要抢本进程 GIL;GIL 若正被 GUI/WSOLA 生产者线程持有、而该线程
   被 pc-service 尖峰抢走 CPU(**优先级反转**),回调只能干等。设备缓冲 160ms 是按"扛住播放器
   自己的 GUI 尖刺"选的(见十三/十五节),没给外部进程挤压留余量 → 欠载"咯噔"。
**修法(三处,均已实测验证)**:
1. **播放器进程提优先级**:`start_player` 的 `creationflags` 加 `ABOVE_NORMAL_PRIORITY_CLASS
   (0x8000)`——整进程(含 GIL 持有线程)高于 pc-service,前台加成压不过,反转消失。
   验证:psutil 读子进程 priority class = 32768。
2. **pc-service 尖峰线程降优先级**:新增 `_lower_thread_priority()`(BELOW_NORMAL),Tk UI 线程
   (`_ui_thread_main`)与扫描 worker(`_do_scan`,纯 Python 3DES 解 QRC 烧核)线程体开头调用——
   突发只用空闲 CPU,重负载下开窗慢零点几秒可接受。**坑:ctypes 必须显式声明
   `GetCurrentThread.restype=c_void_p` + `SetThreadPriority.argtypes`**——伪句柄 -2 按默认 32 位
   int 传参在 64 位下是无效句柄,调用静默失败(实测 ok=0);声明后 ok=1、优先级读回 -1。
3. **变调重采样向量化**(`audio_engine._produce`):原"连续重采样"是逐样本 Python 循环
   (4096 样本跑几千轮解释器,变调时生产者最大 CPU 头),改 numpy 向量化线性插值,**提速 ~50×**,
   生产者余量大增、更抗挤压。原调(semitones=0)直通不走此段,不受影响。
   坑:向量化时临时变量起名 `pos` 覆盖了外层源位置变量 → `_prod_pos` 被写成数组,headless 测试
   当场抓出,已改名 `rp`。
**验证(无声 headless)**:①新旧重采样 7 档升降调 × 40 随机块(含跨块余量/空块)数值 allclose;
②AudioEngine 升 2 半音端到端:主频 494.0Hz(期望 493.9)、时长 5.98s/6s、最大样本跳变 0.035、
中段零丢块。作者试听待直播实测。
