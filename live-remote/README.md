...
- **礼物菜单(2026-08-05)**:播放器绿幕**左侧竖排"礼物→权益"引导条**(抖音礼物图标 + 自定义文字,如
  ...
  经 `gifts <json>` 推播放器。③**显隐/位置/样式**:WS `{"cmd":"gifts_toggle"}` 翻 `STATE["gifts_visible"]` 发
  `gifts_show 0/1`;播放器 `G` 键显隐、**鼠标单独拖动**礼物条(命中检测)、样式(尺寸/描边宽/间距/描边色)在
  下述「绿幕样式控制」窗调;`gifts_show/gift_x/gift_y/gift_scale/gift_outline/gift_gap/gift_color` 经 STATE 回读缓存;
  拉起播放器时统一重推。手机遥控页"窗口开关"区加"礼物菜单 显示/隐藏"、托盘加"礼物菜单显示"勾选项。**绿幕:礼物图/
  文字各自描一圈边(剪影法,描边色不透明可选),不用底板**(彩色半透明 PNG/白字裸贴绿会留绿边,见 karaoke-player
  README「关键技术决策」;初版用不透明底板,08-06 改剪影描边去黑底)。
- **绿幕样式控制窗(2026-08-06)**:托盘「绿幕样式控制」(单实例,`_style_root`,`_open_style_window`)——统一调
  **礼物菜单 / 歌单 / 歌词**三块的样式:礼物(尺寸/描边宽/间距/描边色)、**歌单**(字号/描边宽/描边色/**左右边距**)、
  **歌词**(字号/描边宽/描边色/**左右边距**)。滑块拖动 live 预览、松手/选色即存,统一走 `set_style(key,v,save)`
  (查 `_GP_CMD` 得播放器 IPC 命令名、`_GP_RANGE` 夹取)→ IPC 推播放器 + `_STYLE_KEYS` 全部 STATE 回读/`start_player`
  重推/跨重启缓存。**左右边距 = 离窗口两侧距离**(歌单/歌词都是水平居中组件,边距即居中带宽度)。**歌单竖直位置改
  播放器内鼠标拖动**(仅纵向、上不越顶下不压歌词,横向固定居中;命中检测,`setlist_y` 经 STATE 回读缓存);歌词固定底部。
