...
- **托盘**:pystray 菜单 = `遥控地址`、`Studio One MIDI`、`曲库: N 首 — 点击管理`(**可点击**→ 曲库管理窗:tkinter 滚动帧(Canvas+右侧滚动条),**每行独立 Frame:斑马纹交替底色 + 悬停高亮 + 点击/播放选中态**(纯 tk 手动画,无 Treeview),**行首"勾选框"=加入歌单**(默认不选中;选中的歌进播放器顶端滚动字幕),**行末带"编辑"/"播放"按钮**——编辑=改歌名歌手,播放=`k_play_mid` 立即 load+play(**有歌在播则切歌**,静默不弹窗),待命名歌名标红)、`K歌歌词`(显隐勾选)、`退出`。**"退出"会先弹 Windows 确认框(是/否)**,防误点——退出不可逆(服务、子进程、监听全收尾)。
