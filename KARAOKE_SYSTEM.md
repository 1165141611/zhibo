...
| 歌名来源 | QRC 的 `[ti:]`/`[ar:]`,入库时清洗(去 `-歌手-ktv` 后缀);脏到救不回的(纯数字ID/空)标 `needs_name` **手动改名** | WeSing 对 KTV/上传版常填脏(成都存成 `2422569`)。干净名只在 `KSongsDataInfo.dat`,但那是 **AES 强加密(熵7.9/非ECB/DLL无TEA或明文key)=弯路,别再啃**(2026-07-14 已验证放弃) |
...
| 歌词排布 | **KTV 上下两行交替、左右错开、同尺寸**(偶行上左/奇行下右);当前行逐字高亮(由白染蓝),另行提前候着;**字体 Q 键循环7款+缓存**;主题**底部锚定**,顶端加**横向滚动歌单**(曲库勾选,O 显隐/Ctrl↑↓ 移/缓存) | 2026-07-14 改;不再上大下小 |
| 前奏/间奏 | KTV 引导圆点(放**即将唱那行的顶部**、与行首对齐,倒计时逐个熄灭无缝接高亮) | |
| 音准线 | 压扁成上部一条扁平窄带(高≤两行歌词),只示旋律趋势;**可遥控显隐 + 跨重启缓存**(手机遥控页开关) | 2026-07-14 由半屏高压扁,并加显隐开关 |
