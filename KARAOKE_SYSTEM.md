...
| 每曲默认调式 | 曲库管理页每行 **`−  <调>  +`** 设该曲默认调(存 `library.key`,[-12,12],0=原调);载歌时 `_player_load` 自动下发,**手机点到这首就是调好的调,不必每次手动升降**。手机端 `key` 跟随服务端推的 `k_key`,自动显示、无需改 App | 2026-07-16 新增;2026-08-06 范围由 [-6,6] 扩到 [-12,12](三端同扩:player/`library.set_key`/`_bump_key`/App `keyDelta`),大幅度变调音质会下降,按需使用 |
...
| 歌词排布 | **KTV 上下两行交替、左右错开、同尺寸**(偶行上左/奇行下右);当前行逐字擦除**经典 KTV 双色描边**(未唱=白底黑描边,已唱=蓝底白描边+黑 keyline);另行提前候着;**字体 Q 键循环7款+缓存**;主题**底部锚定**(固定底部间距,不调位置),顶端加**水平居中带横向滚动歌单**(曲库勾选,O 显隐 / **鼠标竖直拖动**或 Ctrl↑↓ 移 / 缓存)。**歌单与歌词样式可调**(绿幕样式控制窗:字体大小/描边宽/描边色/左右边距=居中带宽度) | 2026-07-14 双行错开;2026-07-15 换经典 KTV 双色描边;2026-08-06 歌单改鼠标竖直拖动+居中带 + 歌单/歌词样式做成可调 |
...
| 礼物菜单 | 绿幕**左侧竖排"礼物→权益"引导条**(抖音礼物图标 + 自定义文字,如 🎈点歌/🍰插队),静态展示引导打赏。**无底板,图标/文字各自描一圈黑边**(取内容剪影填黑、8 向偏移画一圈 = 贴纸式黑 keyline,彩色半透明礼物 PNG/白字裸贴绿会留绿边;emoji 走剪影也能描边),预合成 pixmap 只 blit。目录抓抖音 `gift/list?aid=1128` 缓存;**pc-service 托盘配置窗**选礼物+填文字+排序+**「样式」面板(尺寸/描边粗细/间距/描边颜色取色器)**;**`G` 显隐 + 鼠标单独拖动摆位 + 样式实时调**;显隐/位置/样式/内容跨重启缓存 + 手机遥控显隐 | 2026-08-05 加;08-06 由不透明底板改剪影描边去黑底 + 描边宽/颜色/间距/尺寸做成配置窗可调项(描边色不透明,淡化靠选浅灰)。绿幕坑同歌词黑 keyline 原理;抓取/配置在 pc-service(`gifts.py`+配置窗),渲染/拖动/缩放在播放器 |
