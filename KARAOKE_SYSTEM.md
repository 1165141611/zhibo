...
| 礼物菜单 | 绿幕**左侧竖排"礼物→权益"引导条**(抖音礼物图标 + 自定义文字,如 🎈点歌/🍰插队),静态展示引导打赏。每礼物坐一张**不透明深色圆角底板 + 黑 keyline**卡片(彩色半透明礼物 PNG 裸贴绿会留绿边,必须坐底板;emoji 走 drawText 出彩色),预合成 pixmap 只 blit。目录抓抖音 `gift/list?aid=1128` 缓存;**pc-service 托盘配置窗**选礼物+填文字+排序;**`G` 显隐 + 鼠标单独拖动摆位**(命中检测 vs 拖整窗);显隐/位置/内容跨重启缓存 + 手机遥控显隐 | 2026-08-05 加。绿幕坑同歌词黑 keyline 原理;抓取/配置在 pc-service(`gifts.py`+配置窗),渲染/拖动在播放器 |
